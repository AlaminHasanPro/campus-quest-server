...
async function run() {
  try {
    ...
    const collegesCollection = client.db("campusQuest").collection("colleges");
    const graduatedGalleryCollection = client
      .db("campusQuest")
      .collection("graduatedGallery");

    app.get("/colleges", async (req, res) => {
      const colleges = collegesCollection.find();
      const result = await colleges.toArray();
      res.send(result);
    });
    app.get("/graduatedGallery", async (req, res) => {
      const photos = graduatedGalleryCollection.find();
      const result = await photos.toArray();
      res.send(result);
    });
    app.post("/colleges", async (req, res) => {
      const colleges = req.body;
      const result = await collegesCollection.insertOne(colleges);
      res.send(result);
    });

    app.get("/colleges2", async (req, res) => {
      const college = collegesCollection.find().limit(3);
